package ai.corca.generator_placement.infrastructure

import ai.corca.generator_placement.auth.ApiKeyAuth
import ai.corca.generator_placement.auth.HttpBearerAuth
...
class ApiClient(
    ...
    constructor(
        ...
        authNames.forEach { authName ->
            val auth = when (authName) {
                "api-key" -> ApiKeyAuth("header", "X-Api-Key")"accessToken" -> HttpBearerAuth("bearer")
    ...
    companion object {
        @JvmStatic
        protected val baseUrlKey = "ai.corca.generator_placement.baseUrl"

        @JvmStatic
        val defaultBasePath: String by lazy {
            System.getProperties().getProperty(baseUrlKey, "https://api.adcio.ai")
        }
    }
}
